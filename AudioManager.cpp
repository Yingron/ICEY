/*管理bgm、音效、ui音效和语音，需要播放的音频需要在AudioManager::initDefaultConfigs()中注册，然后在需要的地方调用：
...
AudioManager::AudioManager()
{
    ...
    // 注册音频结束回调
    // AudioEngine::setFinishCallback会在音频播放完毕后调用
}
...
{
    if (m_isMuted == muted) return;  // 避免重复操作
    m_isMuted = muted;
...
int AudioManager::playVoice(const std::string& audioId)//语音播放接口
{
    ...
    // 2. 降低BGM音量以突出语音
    float duckedBGMVolume = originalBGMVolume * 0.3f;  // 降低30%
    if (m_currentBGMAudioID != -1) {
...
void AudioManager::fadeInBGM(const std::string& audioId, float duration)
{
    ...
    if (m_currentBGMAudioID != AudioEngine::INVALID_AUDIO_ID) {
        ...
        // 创建一个淡入动作
        // 注意：这里使用简单的线性淡入，实际项目中可以使用更复杂的插值函数
        float step = targetVolume / (duration * 60.0f); // 假设60FPS

        // 由于AudioEngine没有内置的淡入淡出功能，我们可以使用schedule来实现
        // 这里使用一个简单的schedule来实现淡入效果
...
void AudioManager::fadeOutBGM(float duration)
{
    ...
    float currentVolume = AudioEngine::getVolume(m_currentBGMAudioID);
    float step = currentVolume / (duration * 60.0f); // 假设60FPS

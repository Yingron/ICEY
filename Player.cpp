...
bool Player::init(const std::string& spriteFile) {
    ...
    // 初始化攻击间隔相关变量
    _attackCooldown = 0.5f; // 攻击间隔为0.5秒
...
void Player::toggleInvincibleMode() {
    ...
    // 保存或恢复原始最大生命值
    if (_invincibleModeEnabled) {
        // 启用无敌模式：保存原始生命值，并将当前生命值设为极大值
        _originalMaxHealth = _maxHealth;
        _originalCurrentHealth = _currentHealth;
        _displayHealth = _originalCurrentHealth; // 保持显示生命值不变
        _maxHealth = 999999; // 设置一个极大值作为无敌状态的生命值
        _currentHealth = _maxHealth;
        log("无敌模式已启用");
    }
    else {
...
float Player::getHealthPercentage() const {
    // 无敌模式下使用_displayHealth和_originalMaxHealth计算百分比
    if (_invincibleModeEnabled) {
...
void Player::testInvincibleMode() {
    ...
    log("无敌模式测试: %s", _invincibleModeEnabled ? "已开启" : "已关闭");
    log("当前生命值: %.0f, 最大生命值: %.0f", getCurrentHealth(), getMaxHealth());
    log("显示生命值: %.0f, 生命值百分比: %.2f%%", getDisplayHealth(), getHealthPercentage() * 100);
}
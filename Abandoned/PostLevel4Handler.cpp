...
void PostLevel4Handler::createUI() {
    ...
    _countdownLabel = Label::createWithSystemFont(
        StringUtils::format("锟斤拷锟斤拷时: %.1f 锟斤拷", _countdownTimer), 
        "Arial", 
        36
    );
    if (_countdownLabel) {
...
void PostLevel4Handler::updateUI() {
    if (_countdownLabel) {
        // 锟斤拷锟接帮拷全锟斤拷椋凤拷锟斤拷锟绞憋拷锟街碉拷锟叫?
        float displayTime = std::isnan(_countdownTimer) ? 30.0f : _countdownTimer;
        _countdownLabel->setString(StringUtils::format("锟斤拷锟斤拷时: %.1f 锟斤拷", displayTime));
